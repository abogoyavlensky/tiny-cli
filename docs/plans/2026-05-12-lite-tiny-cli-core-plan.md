...
- `run!` is the convenience runner that accepts normalized CLI args, prints, invokes handlers, and exits for help, version, and user-facing errors.
...
Use portable `.cljc` for the pure API. Host-specific output and exit behavior belongs only in `run!` helpers; callers own process argv normalization. Use reader conditionals with a `:lg` branch for let-go and a `:default` branch for Clojure/Babashka where needed, so Clojure and Babashka can load the namespace without seeing let-go-only symbols. If string helpers are required in the namespace form, use reader-conditional requires: let-go's embedded `string` namespace in the `:lg` branch and `clojure.string` in the `:default` branch.
...
- [x] **Step 1: Write failing built-in and runner tests**
...
- [x] **Step 3: Implement built-in parse results**
...
- [x] **Step 4: Implement host-specific `run!`**
  Add reader-conditional host helpers for writing output/error and exiting. Callers pass normalized CLI args directly to `run!`; the library does not inspect `os/args`, `*command-line-args*`, bundle mode, or script paths. In let-go, use `write!` and `os/exit`. In the `:default` branch, use `print`, `binding` to `*err*`, and `System/exit`. `run!` interprets `parse` results, calls command handlers for `:ok`, prints help/version to stdout, prints errors to stderr, and exits non-zero for errors.
...
- host-specific output/exit behavior for `run!` in Let-Go and default Clojure/Babashka branches
